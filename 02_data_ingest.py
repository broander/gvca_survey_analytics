...
def inspect_header(conn):
    ...
    for i, (question, sub_question) in enumerate(
        zip(raw_header, raw_sub_header)
    ):
        current_question = (
            question if question else current_question
        )  # if it's blank, use the last question that wasn't blank
...
def fix_questions(conn, questions):
    ...
    for i in [12, 24, 25, 44, 45, 65, 66, 67, 92, 104, 105, 123]:
        if (
            questions[i]["question description"]
    ...
    for question_group in open_response_questions:
        for i_order, i in enumerate(question_group):
            ...
            if (
                questions[i]["question description"]
                == "Responses pertinent to Grammar School only"
            ):
                questions[i]["question context"] = "Grammar School"
            elif (
                questions[i]["question description"]
                == "Responses pertinent to Middle School only"
            ):
                questions[i]["question context"] = "Middle School"
            elif (
                questions[i]["question description"]
                == "Responses pertinent to High School only"
            ):
                questions[i]["question context"] = "High School"
            elif (
                questions[i]["question description"]
                == "Responses generic to the whole school."
            ):
    ...
    question_info_from_db = conn.execute(
        text(
            f"SELECT question_id, question_type, question_text FROM {DATABASE_SCHEMA}.questions;"
        )
    ).fetchall()
    ...
    for i, q in questions.items():
        results = [
            ...
            for question_id, question_type, question_text in question_info_from_db
...
def validate_fixed_questions(questions):
    ...
    for indexes, text, text_type in [
        ...
        ([0], "Respondent ID", "question description"),
        ([1], "Collector ID", "question description"),
        ([2], "Start Date", "question description"),
        ([3], "End Date", "question description"),
        ([9], "Choose a method of submission.", "question description"),
        (
            [10],
        ...
        ([9, 10, 134, 135], "Response", "question context"),
        ([133], "Open-Ended Response", "question context"),
        (
            [
                *range(11, 18),
                *range(18, 38, 2),
        ...
        assert all(
            text == questions[i][text_type] for i in indexes
        ), (  # compose a useful error message
            f'Expected {text_type} = "{text}"\n\t'
            + '"\n\t'.join(
                [
                    f'column {i}: "{questions[i]["question description"]}'
                    for i in indexes
...
def get_question_response(
    questions: dict, question_description: str, response_row: dict
) -> int:
    """
    ...
    for i in [
        question_index
        for question_index, question_values in questions.items()
        if question_values["question description"] == question_description
    ]:
        if response_row[i]:
...
def main():
    ...
    with open(INPUT_FILEPATH, "r") as f_in, eng.connect() as conn:
        ...
        for i, row in enumerate(raw_data_reader):
            ...
            for question_id, question_type, question_text in conn.execute(
                text(
                    f"""SELECT question_id, question_type, question_text FROM
                     {DATABASE_SCHEMA}.questions;"""
                )
            ).all():
                logging.info(f"Processing question {question_id} for row {i}")
                if question_type == "rank":
                    populate_rank_response(
                        conn, question_id, question_text, raw_questions, row
                    )
                if question_type == "open response":
                    populate_open_response(
                        conn, question_id, question_text, raw_questions, row
                    )
...
def populate_respondents(conn, questions, row):
    ...
    grammar_rank_questions = [
        convert_to_int(row[i])
        for i, q in questions.items()
        if q["question context"] == "Grammar School"
        and row[i]
        and q["question type"] == "rank"
    ]
    middle_rank_questions = [
        convert_to_int(row[i])
        for i, q in questions.items()
        if q["question context"] == "Middle School"
        and row[i]
        and q["question type"] == "rank"
    ]
    high_rank_questions = [
        convert_to_int(row[i])
        for i, q in questions.items()
        if q["question context"] == "High School"
        and row[i]
        and q["question type"] == "rank"
    ]
    all_rank_questions = (
        grammar_rank_questions + middle_rank_questions + high_rank_questions
    )
    add_to_table(
        conn,
        ...
        grammar_avg=(
            sum(grammar_rank_questions) / len(grammar_rank_questions)
            if len(grammar_rank_questions) > 0
            else None
        ),
        middle_avg=(
            sum(middle_rank_questions) / len(middle_rank_questions)
            if len(middle_rank_questions) > 0
            else None
        ),
        high_avg=(
            sum(high_rank_questions) / len(high_rank_questions)
            if len(high_rank_questions) > 0
            else None
        ),
        overall_avg=(
            sum(all_rank_questions) / len(all_rank_questions)
...
def populate_rank_response(
    ...
    for question, response in zip(raw_questions.values(), row):
        ...
        if question["question description"] == question_text and response:
            logging.debug(
                f'{question["question context"]} response: {response}'
            )
            add_to_table(
                conn,
