...
if [ -f './environment.yml' ]; then
    umask 0002 && /opt/conda/bin/mamba create --name project --clone base && /opt/conda/bin/mamba env update -n project -f ./environment.yml
else
    umask 0002 && /opt/conda/bin/mamba create --name project --clone base
fi
